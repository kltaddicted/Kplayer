...
export class Volume extends Options {
    ...
    initEvent(): void {
        this.player.on(EVENT.VOLUME_PROGRESS_CLICK, (e: MouseEvent, ctx: Volume) => {
            ...
            this.volumeDot.style.top =
                (1 - scale) * this.volumeProgress.clientHeight + 'px'
            this.volume = scale
        ...
        this.volumeDot.onmousedown = (e: MouseEvent) => {
            ...
            document.body.onmousemove = (e: MouseEvent) => {
                ...
                let scale =
                    (this.volumeProgress.clientHeight - dy) /
                    this.volumeProgress.clientHeight
                if (scale < 0) {
                    scale = 0
                } else if (scale > 1) {
                    scale = 1
