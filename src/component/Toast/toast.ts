...
import { ComponentItem, DOMProps } from '../../type/Player'
...
export class Toast extends Component implements ComponentItem {
    ...
    dispose(): void {
        ...
        for (let i = index; i < Toast.ToastQueue.length; i++) {
            ...
            if (i === index) {
                toast.el.style.transform = `translateY(${10 + this.el.clientHeight
                    }px)`
            } else {
                toast.el.style.transform = `translateY(${10 + Toast.ToastQueue[i - 1].el.clientHeight
                    }px)`
            }
        }
    }
